...
{
	nrhs++; 
	AUX[1]=  C[9];  		 /* variable1=w1; */
	AUX[2]=  C[9];  		 /* variable2=w1; */
}
...
int load_data_figures(double FIGS[][MAX_FIGS], eqfig figdata[], int nfigs, int jj, double y[], double C[], double AUX[], double x, FILE * myfile) { 
	 ...
	 int i; int errorover=0; 
		  FIGS[0][j] = x; 
		  if (! finite(AUX[1])) { printf("Overflow error on variable variable1 \n"); errorover++; } 
		  FIGS[1][j] = AUX[1];
		  if (figdata[1].max < FIGS[1][j]) { figdata[1].max =  FIGS[1][j]; } 
		  if (figdata[1].min > FIGS[1][j]) { figdata[1].min =  FIGS[1][j]; } 
		  if (! finite(AUX[2])) { printf("Overflow error on variable variable2 \n"); errorover++; } 
		  FIGS[2][j] = AUX[2];
...
{ 
	 ...
	 for(i=1;i<=nfigs;i++) { figdata[i].max = -HUGE_VAL; figdata[i].min=HUGE_VAL; }; 
	  strcpy(figdata[1].name,"variable1");
	  strcpy(figdata[2].name,"variable2");
	 while (myc < 100) { 
	 load_constants(C); 
	...
	for(i=0; i<nfigsloaded; i++) { 
	 ...
	 myc++; } 
	 WritePlotScript(1, nfigs, 360, figdata, 0, myc-1);
